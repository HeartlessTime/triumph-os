...
def calculate_estimate_totals(
    ...
    return {
        ...
        'material_total': material_total.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP),
...
def recalculate_estimate(estimate: Estimate) -> Estimate:
    ...
    totals = calculate_estimate_totals(
        labor_items,
        material_items,
        Decimal(str(estimate.margin_percent or 20))
    )
...
def get_next_version(opportunity_id: int, db) -> int:
    ...
    max_version = db.query(Estimate.version)\
