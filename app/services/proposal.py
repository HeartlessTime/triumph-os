...
def generate_proposal_pdf(
    ...
    if opportunity.description or opportunity.scopes:
        ...
        if opportunity.scopes:
            scope_text = "<b>Scope Packages:</b> " + ", ".join(s.name for s in opportunity.scopes)
    ...
    if labor_items:
        ...
        # Add subtotal row
        labor_data.append(['', '', '', 'Labor Subtotal:', format_currency(estimate.labor_total)])
        
        labor_table = Table(labor_data, colWidths=[3*inch, 0.7*inch, 0.7*inch, 1*inch, 1.1*inch])
    ...
    # Materials section
    material_items = [item for item in estimate.line_items if item.line_type == 'material']
    if material_items:
        ...
        # Add subtotal row
        material_data.append(['', '', '', 'Materials Subtotal:', format_currency(estimate.material_total)])
        
        material_table = Table(material_data, colWidths=[3*inch, 0.7*inch, 0.7*inch, 1*inch, 1.1*inch])
    ...
    summary_data = [
        ...
        [f'Markup ({estimate.margin_percent}%):', format_currency(estimate.margin_amount)],
