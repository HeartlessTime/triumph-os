...
def calculate_estimator_workload(db: Session, estimator_id: int) -> dict:
    ...
    upcoming_bids = [
        opp for opp in opportunities if opp.bid_date and opp.bid_date <= seven_days_out
    ]
    ...
    opp_details = [
        {
            ...
            "stage": opp.stage,
            "value": (float(opp.lv_value) if opp.lv_value else 0)
            + (float(opp.hdd_value) if opp.hdd_value else 0),
            "bid_date": opp.bid_date,
            "estimating_status": opp.estimating_status,
            "days_until_bid": (opp.bid_date - datetime.now().date()).days
            if opp.bid_date
            else None,
        }
        for opp in opportunities
...
async def estimator_dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    for estimator in estimators:
        ...
        estimator_workloads.append({"estimator": estimator, **workload_data})
...
async def get_estimator_workload_json(
    estimator_id: int, request: Request, db: Session = Depends(get_db)
):
...
async def suggest_estimator_assignment(request: Request, db: Session = Depends(get_db)):
    """
    ...
    if not estimators:
        return JSONResponse(
            {"suggested_estimator_id": None, "reason": "No estimators found"}
        )
    ...
    return JSONResponse(
        {"suggested_estimator_id": None, "reason": "Unable to determine suggestion"}
    )
